...
(function(c, j) {
  function k(a) {
    ...
    c.extend(c.ui, {version:'1.8.11', keyCode:{ALT:18, BACKSPACE:8, CAPS_LOCK:20, COMMA:188, COMMAND:91, COMMAND_LEFT:91, COMMAND_RIGHT:93, CONTROL:17, DELETE:46, DOWN:40, END:35, ENTER:13, ESCAPE:27, HOME:36, INSERT:45, LEFT:37, MENU:93, NUMPAD_ADD:107, NUMPAD_DECIMAL:110, NUMPAD_DIVIDE:111, NUMPAD_ENTER:108, NUMPAD_MULTIPLY:106,
      NUMPAD_SUBTRACT:109, PAGE_DOWN:34, PAGE_UP:33, PERIOD:190, RIGHT:39, SHIFT:16, SPACE:32, TAB:9, UP:38, WINDOWS:91}}); c.fn.extend({_focus:c.fn.focus, focus:function(a, b) {
...
'position', 1)) && /(auto|scroll)/.test(c.curCSS(this, 'overflow', 1) + c.curCSS(this, 'overflow-y', 1) + c.curCSS(this, 'overflow-x', 1));
        ...
        if (a !== j) return this.css('zIndex', a); if (this.length) {
          a = c(this[0]); for (var b; a.length && a[0] !== document;) {
            ...
            b = parseInt(a.css('zIndex'), 10); if (!isNaN(b) && b !== 0) return b;
          }a = a.parent();
          }
        } return 0;
...
'border' + this + 'Width', true)) || 0; if (m)g -= parseFloat(c.curCSS(f, 'margin' + this, true)) || 0;
      ...
      } var e = b === 'Width' ? ['Left', 'Right'] : ['Top', 'Bottom'], h = b.toLowerCase(), i = {innerWidth:c.fn.innerWidth, innerHeight:c.fn.innerHeight, outerWidth:c.fn.outerWidth, outerHeight:c.fn.outerHeight}; c.fn['inner' + b] = function(f) {
...
d(this, f, true, g) + 'px');
    ...
    var b = a.nodeName.toLowerCase(), d = c.attr(a, 'tabindex'); if ('area' === b) {
    ...
    var b = c.attr(a, 'tabindex'); return (isNaN(b) || b >= 0) && c(a).is(':focusable');
  }});
    c(function() {
      var a = document.body, b = a.appendChild(b = document.createElement('div')); c.extend(b.style, {minHeight:'100px', height:'auto', padding:0, borderWidth:0}); c.support.minHeight = b.offsetHeight === 100; c.support.selectstart = 'onselectstart' in b; a.removeChild(b).style.display = 'none';
...
(function(b, j) {
  if (b.cleanData) {
    var k = b.cleanData; b.cleanData = function(a) {
      for (var c = 0, d; (d = a[c]) != null; c++)b(d).triggerHandler('remove'); k(a);
    };
  } else {
...
a);
    ...
    }; c = new c; c.options = b.extend(true, {}, c.options); b[e][a].prototype = b.extend(true, c, {namespace:e, widgetName:a, widgetEventPrefix:b[e][a].prototype.widgetEventPrefix || a, widgetBaseClass:f}, d); b.widget.bridge(a, b[e][a]);
  ...
  b.widget('ui.mouse', {options:{cancel:':input,option', distance:1, delay:0}, _mouseInit:function() {
    var a = this; this.element.bind('mousedown.' + this.widgetName, function(c) {
      ...
      if (true === b.data(c.target, a.widgetName + '.preventClickEvent')) {
        b.removeData(c.target, a.widgetName + '.preventClickEvent'); c.stopImmediatePropagation(); return false;
      }
    }); this.started = false;
...
a.originalEvent || {}; if (!a.originalEvent.mouseHandled) {
  this._mouseStarted && this._mouseUp(a); this._mouseDownEvent = a; var c = this, e = a.which == 1, f = typeof this.options.cancel == 'string' ? b(a.target).parents().add(a.target).filter(this.options.cancel).length : false; if (!e || f || !this._mouseCapture(a)) return true; this.mouseDelayMet = !this.options.delay; if (!this.mouseDelayMet) this._mouseDelayTimer = setTimeout(function() {
    c.mouseDelayMet = true;
  }, this.options.delay); if (this._mouseDistanceMet(a) && this._mouseDelayMet(a)) {
    this._mouseStarted =
this._mouseStart(a) !== false; if (!this._mouseStarted) {
  a.preventDefault(); return true;
}
  }true === b.data(a.target, this.widgetName + '.preventClickEvent') && b.removeData(a.target, this.widgetName + '.preventClickEvent'); this._mouseMoveDelegate = function(d) {
    return c._mouseMove(d);
  }; this._mouseUpDelegate = function(d) {
    return c._mouseUp(d);
  }; b(document).bind('mousemove.' + this.widgetName, this._mouseMoveDelegate).bind('mouseup.' + this.widgetName, this._mouseUpDelegate); a.preventDefault(); return a.originalEvent.mouseHandled =
true;
}
  }, _mouseMove:function(a) {
    if (b.browser.msie && !(document.documentMode >= 9) && !a.button) return this._mouseUp(a); if (this._mouseStarted) {
      this._mouseDrag(a); return a.preventDefault();
    ...
    if (this._mouseStarted) {
      this._mouseStarted = false; a.target == this._mouseDownEvent.target && b.data(a.target, this.widgetName + '.preventClickEvent', true); this._mouseStop(a);
    } return false;
...
(function(c) {
  c.ui = c.ui || {}; var n = /left|center|right/, o = /top|center|bottom/, t = c.fn.position, u = c.fn.offset; c.fn.position = function(b) {
    if (!b || !b.of) return t.apply(this, arguments); b = c.extend({}, b); var a = c(b.of), d = a[0], g = (b.collision || 'flip').split(' '), e = b.offset ? b.offset.split(' ') : [0, 0], h, k, j; if (d.nodeType === 9) {
      h = a.width(); k = a.height(); j = {top:0, left:0};
    } else if (d.setTimeout) {
      h = a.width(); k = a.height(); j = {top:a.scrollTop(), left:a.scrollLeft()};
    } else if (d.preventDefault) {
      b.at = 'left top'; h = k = 0; j = {top:b.of.pageY,
        left:b.of.pageX};
    } else {
      h = a.outerWidth(); k = a.outerHeight(); j = a.offset();
    }c.each(['my', 'at'], function() {
      var f = (b[this] || '').split(' '); if (f.length === 1)f = n.test(f[0]) ? f.concat(['center']) : o.test(f[0]) ? ['center'].concat(f) : ['center', 'center']; f[0] = n.test(f[0]) ? f[0] : 'center'; f[1] = o.test(f[1]) ? f[1] : 'center'; b[this] = f;
    }); if (g.length === 1)g[1] = g[0]; e[0] = parseInt(e[0], 10) || 0; if (e.length === 1)e[1] = e[0]; e[1] = parseInt(e[1], 10) || 0; if (b.at[0] === 'right')j.left += h; else if (b.at[0] === 'center')j.left += h / 2; if (b.at[1] === 'bottom')j.top +=
...
m / 2; i.left = Math.round(i.left); i.top = Math.round(i.top); r = {left:i.left - p, top:i.top - q}; c.each(['left', 'top'], function(s, x) {
...
}); c.fn.bgiframe && f.bgiframe(); f.offset(c.extend(i, {using:b.using}));
});
...
d > 0 ? b.left - d : Math.max(b.left - a.collisionPosition.left, b.left);
    ...
    var d = c(window); d = a.collisionPosition.top + a.collisionHeight - d.height() - d.scrollTop(); b.top = d > 0 ? b.top - d : Math.max(b.top - a.collisionPosition.top, b.top);
...
a.collisionPosition.left < 0 ? g + e + h : d > 0 ? g + e + h : 0;
    ...
    if (a.at[1] !== 'center') {
      var d = c(window); d = a.collisionPosition.top + a.collisionHeight - d.height() - d.scrollTop(); var g = a.my[1] === 'top' ? -a.elemHeight : a.my[1] === 'bottom' ? a.elemHeight : 0, e = a.at[1] === 'top' ? a.targetHeight : -a.targetHeight, h = -2 * a.offset[1]; b.top += a.collisionPosition.top < 0 ? g + e + h : d > 0 ? g + e + h : 0;
    }
...
this.margins.top, left:this.offset.left - this.margins.left}; d.extend(this.offset, {click:{left:a.pageX - this.offset.left, top:a.pageY - this.offset.top}, parent:this._getParentOffset(), relative:this._getRelativeOffset()}); this.originalPosition = this.position = this._generatePosition(a); this.originalPageX = a.pageX; this.originalPageY = a.pageY; b.cursorAt && this._adjustOffsetFromHelper(b.cursorAt); b.containment && this._setContainment(); if (this._trigger('start', a) === false) {
  this._clear(); return false;
    ...
    this.position = this._generatePosition(a); this.positionAbs = this._convertPositionTo('absolute'); if (!b) {
...
this.options.axis != 'x') this.helper[0].style.top = this.position.top + 'px'; d.ui.ddmanager && d.ui.ddmanager.drag(this, a); return false;
    ...
    var b = false; if (d.ui.ddmanager && !this.options.dropBehaviour)b = d.ui.ddmanager.drop(this, a); if (this.dropped) {
      b = this.dropped; this.dropped = false;
    } if ((!this.element[0] || !this.element[0].parentNode) && this.options.helper == 'original') return false; if (this.options.revert == 'invalid' && !b || this.options.revert == 'valid' && b || this.options.revert === true || d.isFunction(this.options.revert) &&
...
this.offsetParent[0])) {
  ...
  } if (this.offsetParent[0] == document.body || this.offsetParent[0].tagName && this.offsetParent[0].tagName.toLowerCase() == 'html' && d.browser.msie)a = {top:0, left:0}; return {top:a.top + (parseInt(this.offsetParent.css('borderTopWidth'), 10) || 0), left:a.left + (parseInt(this.offsetParent.css('borderLeftWidth'), 10) || 0)};
...
f ? 0 : c.scrollLeft())};
  ...
  c = c || this._uiHash(); d.ui.plugin.call(this, a, [b, c]); if (a == 'drag') this.positionAbs = this._convertPositionTo('absolute'); return d.Widget.prototype._trigger.call(this, a, b, c);
}, plugins:{}, _uiHash:function() {
  return {helper:this.helper, position:this.position, originalPosition:this.originalPosition,
    offset:this.positionAbs};
}}); d.extend(d.ui.draggable, {version:'1.8.11'}); d.ui.plugin.add('draggable', 'connectToSortable', {start:function(a, b) {
  var c = d(this).data('draggable'), f = c.options, e = d.extend({}, b, {item:c.element}); c.sortables = []; d(f.connectToSortable).each(function() {
    var g = d.data(this, 'sortable'); if (g && !g.options.disabled) {
      c.sortables.push({instance:g, shouldRevert:g.options.revert}); g.refreshPositions(); g._trigger('activate', a, e);
    }
  });
}, stop:function(a, b) {
  var c = d(this).data('draggable'), f = d.extend({},
b, {item:c.element}); d.each(c.sortables, function() {
  if (this.instance.isOver) {
    this.instance.isOver = 0; c.cancelHelperRemoval = true; this.instance.cancelHelperRemoval = false; if (this.shouldRevert) this.instance.options.revert = true; this.instance._mouseStop(a); this.instance.options.helper = this.instance.options._helper; c.options.helper == 'original' && this.instance.currentItem.css({top:'auto', left:'auto'});
  } else {
...
d(this).data('draggable'), f = this; d.each(c.sortables, function() {
  this.instance.positionAbs = c.positionAbs; this.instance.helperProportions = c.helperProportions; this.instance.offset.click = c.offset.click; if (this.instance._intersectsWith(this.instance.containerCache)) {
    if (!this.instance.isOver) {
      this.instance.isOver = 1; this.instance.currentItem = d(f).clone().appendTo(this.instance.element).data('sortable-item', true); this.instance.options._helper = this.instance.options.helper; this.instance.options.helper = function() {
        return b.helper[0];
      };
...
this.instance._mouseDrag(a);
  ...
  {start:function() {
    var a = d('body'), b = d(this).data('draggable').options; if (a.css('cursor'))b._cursor = a.css('cursor'); a.css('cursor', b.cursor);
  }, stop:function() {
...
'px', height:this.offsetHeight + 'px', position:'absolute', opacity:'0.001', zIndex:1E3}).css(d(this).offset()).appendTo('body');
    ...
    d('div.ui-draggable-iframeFix').each(function() {
      ...
      a = d(b.helper); b = d(this).data('draggable').options; if (a.css('opacity'))b._opacity = a.css('opacity'); a.css('opacity', b.opacity);
    }, stop:function(a, b) {
    a = d(this).data('draggable').options; a._opacity && d(b.helper).css('opacity',
a._opacity);
  }}); d.ui.plugin.add('draggable', 'scroll', {start:function() {
    var a = d(this).data('draggable'); if (a.scrollParent[0] != document && a.scrollParent[0].tagName != 'HTML')a.overflowOffset = a.scrollParent.offset();
  }, drag:function(a) {
...
c.scrollSpeed);
  ...
  var a = d(this).data('draggable'), b = a.options; a.snapElements = []; d(b.snap.constructor != String ? b.snap.items || ':data(draggable)' : b.snap).each(function() {
    var c = d(this), f = c.offset(); this != a.element[0] && a.snapElements.push({item:this, width:c.outerWidth(), height:c.outerHeight(), top:f.top, left:f.left});
  });
}, drag:function(a, b) {
...
{snapItem:c.snapElements[h].item})); c.snapElements[h].snapping = p || q || r || s || t;
      ...
      c.snapElements[h].snapping && c.options.snap.release && c.options.snap.release.call(c.element, a, d.extend(c._uiHash(), {snapItem:c.snapElements[h].item})); c.snapElements[h].snapping = false;
    }
  }
  ...
  var a = d(this).data('draggable').options; a = d.makeArray(d(a.stack)).sort(function(c, f) {
    ...
    var b =
parseInt(a[0].style.zIndex) || 0; d(a).each(function(c) {
  this.style.zIndex = b + c;
}); this[0].style.zIndex = b + a.length;
  }
}}); d.ui.plugin.add('draggable', 'zIndex', {start:function(a, b) {
  a = d(b.helper); b = d(this).data('draggable').options; if (a.css('zIndex'))b._zIndex = a.css('zIndex'); a.css('zIndex', b.zIndex);
}, stop:function(a, b) {
...
(function(d) {
  d.widget('ui.droppable', {widgetEventPrefix:'drop', options:{accept:'*', activeClass:false, addClasses:true, greedy:false, hoverClass:false, scope:'default', tolerance:'intersect'}, _create:function() {
    var a = this.options, b = a.accept; this.isover = 0; this.isout = 1; this.accept = d.isFunction(b) ? b : function(c) {
      return c.is(b);
    }; this.proportions = {width:this.element[0].offsetWidth, height:this.element[0].offsetHeight}; d.ui.ddmanager.droppables[a.scope] = d.ui.ddmanager.droppables[a.scope] || []; d.ui.ddmanager.droppables[a.scope].push(this);
    a.addClasses && this.element.addClass('ui-droppable');
  }, destroy:function() {
    for (var a = d.ui.ddmanager.droppables[this.options.scope], b = 0; b < a.length; b++)a[b] == this && a.splice(b, 1); this.element.removeClass('ui-droppable ui-droppable-disabled').removeData('droppable').unbind('.droppable'); return this;
...
this.element.addClass(this.options.activeClass); b && this._trigger('activate', a, this.ui(b));
    ...
    var b = d.ui.ddmanager.current; this.options.activeClass && this.element.removeClass(this.options.activeClass); b && this._trigger('deactivate', a, this.ui(b));
  }, _over:function(a) {
    ...
    var b = d.ui.ddmanager.current; if (!(!b || (b.currentItem || b.element)[0] == this.element[0])) if (this.accept.call(this.element[0], b.currentItem || b.element)) {
      this.options.hoverClass && this.element.removeClass(this.options.hoverClass); this._trigger('out', a, this.ui(b));
    }
  }, _drop:function(a, b) {
    ...
    var g =
d.data(this, 'droppable'); if (g.options.greedy && !g.options.disabled && g.options.scope == c.options.scope && g.accept.call(g.element[0], c.currentItem || c.element) && d.ui.intersect(c, d.extend(g, {offset:g.element.offset()}), g.options.tolerance)) {
  e = true; return false;
}
...
'none'; if (c[f].visible) {
  e == 'mousedown' && c[f]._activate.call(c[f], b); c[f].offset = c[f].element.offset(); c[f].proportions = {width:c[f].element[0].offsetWidth, height:c[f].element[0].offsetHeight};
}
  }
}, drop:function(a, b) {
...
d.data(g[0], 'droppable'); e.greedyChild = c == 'isover' ? 1 : 0;
        ...
        } this[c] = 1; this[c == 'isout' ? 'isover' : 'isout'] = 0; this[c == 'isover' ? '_over' : '_out'].call(this, b); if (e && c == 'isout') {
...
this.originalElement.css('resize'); this.originalElement.css('resize', 'none'); this._proportionallyResizeElements.push(this.originalElement.css({position:'static', zoom:1, display:'block'})); this.originalElement.css({margin:this.originalElement.css('margin')}); this._proportionallyResize();
        ...
        nw:'.ui-resizable-nw'}); if (this.handles.constructor == String) {
          if (this.handles == 'all') this.handles = 'n,e,s,w,se,sw,ne,nw'; var c = this.handles.split(','); this.handles = {}; for (var d = 0; d < c.length; d++) {
            var f = e.trim(c[d]), g = e('<div class="ui-resizable-handle ' + ('ui-resizable-' + f) + '"></div>'); /sw|se|ne|nw/.test(f) && g.css({zIndex:++a.zIndex}); 'se' == f && g.addClass('ui-icon ui-icon-gripsmall-diagonal-se'); this.handles[f] = '.ui-resizable-' + f; this.element.append(g);
          }
...
String) this.handles[i] = e(this.handles[i], this.element).show(); if (this.elementIsWrapper && this.originalElement[0].nodeName.match(/textarea|input|select|button/i)) {
  var j = e(this.handles[i], this.element), k = 0; k = /sw|ne|nw|se|n|s/.test(i) ? j.outerHeight() : j.outerWidth(); j = ['padding', /ne|nw|n/.test(i) ? 'Top' : /se|sw|s/.test(i) ? 'Bottom' : /^e$/.test(i) ? 'Right' : 'Left'].join(''); h.css(j, k); this._proportionallyResize();
}e(this.handles[i]);
          }
        }; this._renderAxis(this.element); this._handles = e('.ui-resizable-handle', this.element).disableSelection();
    this._handles.mouseover(function() {
      if (!b.resizing) {
        if (this.className) var h = this.className.match(/ui-resizable-(se|sw|ne|nw|n|e|s|w)/i); b.axis = h && h[1] ? h[1] : 'se';
      }
    }); if (a.autoHide) {
      this._handles.hide(); e(this.element).addClass('ui-resizable-autohide').hover(function() {
        e(this).removeClass('ui-resizable-autohide'); b._handles.show();
      }, function() {
        if (!b.resizing) {
          e(this).addClass('ui-resizable-autohide'); b._handles.hide();
        }
      });
    } this._mouseInit();
  }, destroy:function() {
    this._mouseDestroy(); var b = function(c) {
      e(c).removeClass('ui-resizable ui-resizable-disabled ui-resizable-resizing').removeData('resizable').unbind('.resizable').find('.ui-resizable-handle').remove();
    };
    if (this.elementIsWrapper) {
      b(this.element); var a = this.element; a.after(this.originalElement.css({position:a.css('position'), width:a.outerWidth(), height:a.outerHeight(), top:a.css('top'), left:a.css('left')})).remove();
    ...
    var a = this.options, c = this.element.position(),
      d = this.element; this.resizing = true; this.documentScroll = {top:e(document).scrollTop(), left:e(document).scrollLeft()}; if (d.is('.ui-draggable') || /absolute/.test(d.css('position')))d.css({position:'absolute', top:c.top, left:c.left}); e.browser.opera && /relative/.test(d.css('position')) && d.css({position:'relative', top:'auto', left:'auto'}); this._renderProxy(); c = m(this.helper.css('left')); var f = m(this.helper.css('top')); if (a.containment) {
...
b); a.css({top:this.position.top + 'px', left:this.position.left + 'px', width:this.size.width + 'px', height:this.size.height + 'px'}); !this._helper && this._proportionallyResizeElements.length && this._proportionallyResize(); this._updateCache(c); this._trigger('resize', b, this.ui()); return false;
    ...
    this.offset = this.helper.offset(); if (l(b.left)) this.position.left = b.left; if (l(b.top)) this.position.top = b.top; if (l(b.height)) this.size.height = b.height; if (l(b.width)) this.size.width = b.width;
...
null;
    ...
    b.top = a.top + (c.height - b.height); b.left = a.left + (c.width - b.width);
  } return b;
...
this.size.height, k = /sw|nw|w/.test(c); c = /nw|ne|n/.test(c); if (g && k)b.left = i - a.minWidth; if (d && k)b.left = i - a.maxWidth; if (h && c)b.top = j - a.minHeight; if (f && c)b.top = j - a.maxHeight; if ((a = !b.width && !b.height) && !b.left && b.top)b.top = null; else if (a && !b.top && b.left)b.left = null; return b;
  ...
  if (this._proportionallyResizeElements.length) for (var b = this.helper || this.element, a = 0; a < this._proportionallyResizeElements.length; a++) {
    var c = this._proportionallyResizeElements[a]; if (!this.borderDif) {
      var d =
[c.css('borderTopWidth'), c.css('borderRightWidth'), c.css('borderBottomWidth'), c.css('borderLeftWidth')], f = [c.css('paddingTop'), c.css('paddingRight'), c.css('paddingBottom'), c.css('paddingLeft')]; this.borderDif = e.map(d, function(g, h) {
...
this.options; this.elementOffset = this.element.offset(); if (this._helper) {
  this.helper = this.helper || e('<div style="overflow:hidden;"></div>'); var a = e.browser.msie && e.browser.version < 7, c = a ? 1 : 0; a = a ? 2 : -1; this.helper.addClass(this._helper).css({width:this.element.outerWidth() + a, height:this.element.outerHeight() + a, position:'absolute', left:this.elementOffset.left - c + 'px', top:this.elementOffset.top - c + 'px', zIndex:++b.zIndex}); this.helper.appendTo('body').disableSelection();
...
a) {
  ...
  e.ui.plugin.call(this, b, [a, this.ui()]); b != 'resize' && this._trigger(b, a, this.ui());
}, plugins:{}, ui:function() {
  ...
  var b = e(this).data('resizable').options, a = function(c) {
      e(c).each(function() {
        var d = e(this); d.data('resizable-alsoresize', {width:parseInt(d.width(), 10), height:parseInt(d.height(), 10), left:parseInt(d.css('left'), 10), top:parseInt(d.css('top'), 10), position:d.css('position')});
      });
...
b.alsoResize[0]; a(b.alsoResize);
      ...
      e(i).each(function() {
        var k = e(this), q = e(this).data('resizable-alsoresize'), p = {}, r = j && j.length ? j : k.parents(a.originalElement[0]).length ? ['width', 'height'] : ['width',
            'height', 'top', 'left']; e.each(r, function(n, o) {
              ...
              c._revertToRelativePosition = true; k.css({position:'absolute', top:'auto', left:'auto'});
            }k.css(p);
      });
  ...
  var b = e(this).data('resizable'), a = b.options, c = function(d) {
      e(d).each(function() {
        var f = e(this); f.css({position:f.data('resizable-alsoresize').position});
      });
    };
...
g}; g = parseInt(a.element.css('left'), 10) + (a.position.left - a.originalPosition.left) || null; var h = parseInt(a.element.css('top'), 10) + (a.position.top - a.originalPosition.top) || null; a.element.animate(e.extend(f, h && g ? {top:h, left:g} : {}), {duration:c.animateDuration, easing:c.animateEasing, step:function() {
  var i = {width:parseInt(a.element.css('width'), 10), height:parseInt(a.element.css('height'), 10), top:parseInt(a.element.css('top'), 10), left:parseInt(a.element.css('left'), 10)}; d && d.length && e(d[0]).css({width:i.width,
    height:i.height}); a._updateCache(i); a._propagate('resize', b);
}});
  ...
  var b = e(this).data('resizable'), a = b.element, c = b.options.containment; if (a = c instanceof e ? c.get(0) : /parent/.test(c) ? a.parent().get(0) : c) {
    b.containerElement = e(a); if (/document/.test(c) || c == document) {
      b.containerOffset = {left:0, top:0}; b.containerPosition = {left:0, top:0}; b.parentData = {element:e(document), left:0, top:0, width:e(document).width(), height:e(document).height() || document.body.parentNode.scrollHeight};
    } else {
      var d =
e(a), f = []; e(['Top', 'Right', 'Left', 'Bottom']).each(function(i, j) {
  ...
  var a = e(this).data('resizable'), c = a.options,
    d = a.containerOffset, f = a.position; b = a._aspectRatio || b.shiftKey; var g = {top:0, left:0}, h = a.containerElement; if (h[0] != document && /static/.test(h.css('position')))g = d; if (f.left < (a._helper ? d.left : 0)) {
...
/static/.test(f.css('position')) && e(this).css({left:h.left - d.left - c.left, width:i, height:g});
  ...
  var b = e(this).data('resizable'), a = b.options, c = b.size; b.ghost = b.originalElement.clone(); b.ghost.css({opacity:0.25, display:'block', position:'relative', height:c.height, width:c.width, margin:0, left:0, top:0}).addClass('ui-resizable-ghost').addClass(typeof a.ghost == 'string' ? a.ghost : ''); b.ghost.appendTo(b.helper);
}, resize:function() {
...
(a.grid[1] || 1)) * (a.grid[1] || 1); if (/^(se|s|e)$/.test(g)) {
...
c, {unselecting:b.element});
          ...
          var b = e.data(this, 'selectable-item'); if (b) {
            var g = !c.metaKey || !b.$element.hasClass('ui-selected'); b.$element.removeClass(g ? 'ui-unselecting' : 'ui-selected').addClass(g ? 'ui-selecting' : 'ui-unselecting'); b.unselecting = !g; b.selecting = g; (b.selected = g) ? f._trigger('selecting', c, {selecting:b.element}) : f._trigger('unselecting', c, {unselecting:b.element}); return false;
          }
        });
      }
...
this.options, b = this.opos[0], g = this.opos[1], h = c.pageX, i = c.pageY; if (b > h) {
  ...
  j = i; i = g; g = j;
} this.helper.css({left:b, top:g, width:h - b, height:i - g}); this.selectees.each(function() {
  var a = e.data(this, 'selectable-item'); if (!(!a || a.element == f.element[0])) {
    var k = false; if (d.tolerance == 'touch')k = !(a.left > h || a.right < b || a.top > i || a.bottom < g); else if (d.tolerance == 'fit')k = a.left > b && a.right < h && a.top > g && a.bottom < i; if (k) {
      if (a.selected) {
        a.$element.removeClass('ui-selected'); a.selected = false;
      } if (a.unselecting) {
        a.$element.removeClass('ui-unselecting');
        a.unselecting = false;
      } if (!a.selecting) {
        a.$element.addClass('ui-selecting'); a.selecting = true; f._trigger('selecting', c, {selecting:a.element});
      }
    } else {
      if (a.selecting) if (c.metaKey && a.startselected) {
        a.$element.removeClass('ui-selecting'); a.selecting = false; a.$element.addClass('ui-selected'); a.selected = true;
      } else {
        a.$element.removeClass('ui-selecting'); a.selecting = false; if (a.startselected) {
          a.$element.addClass('ui-unselecting'); a.unselecting = true;
        }f._trigger('unselecting', c, {unselecting:a.element});
      } if (a.selected) if (!c.metaKey &&
!a.startselected) {
        a.$element.removeClass('ui-selected'); a.selected = false; a.$element.addClass('ui-unselecting'); a.unselecting = true; f._trigger('unselecting', c, {unselecting:a.element});
      }
    }
  }
}); return false;
    ...
    var f = this; this.dragged = false; e('.ui-unselecting', this.element[0]).each(function() {
      var d = e.data(this, 'selectable-item'); d.$element.removeClass('ui-unselecting'); d.unselecting = false; d.startselected = false; f._trigger('unselected', c, {unselected:d.element});
...
(function(d) {
  d.widget('ui.sortable', d.ui.mouse, {widgetEventPrefix:'sort', options:{appendTo:'parent', axis:false, connectWith:false, containment:false, cursor:'auto', cursorAt:false, dropOnEmpty:true, forcePlaceholderSize:false, forceHelperSize:false, grid:false, handle:false, helper:'original', items:'> *', opacity:false, placeholder:false, revert:false, scroll:true, scrollSensitivity:20, scrollSpeed:20, scope:'default', tolerance:'intersect', zIndex:1E3}, _create:function() {
    ...
    this.element.removeClass('ui-sortable ui-sortable-disabled').removeData('sortable').unbind('.sortable'); this._mouseDestroy(); for (var a = this.items.length - 1; a >= 0; a--) this.items[a].item.removeData('sortable-item'); return this;
  }, _setOption:function(a, b) {
    if (a === 'disabled') {
      this.options[a] =
...
e) : true)) {
      this.direction = f == 1 ? 'down' : 'up'; if (this.options.tolerance == 'pointer' || this._intersectsWithSides(c)) this._rearrange(a, c); else break; this._trigger('change', a, this._uiHash()); break;
    }
    ...
    if (a) {
      ...
      c.reverting = true; d(this.helper).animate({left:b.left - this.offset.parent.left - c.margins.left + (this.offsetParent[0] == document.body ? 0 : this.offsetParent[0].scrollLeft), top:b.top - this.offset.parent.top - c.margins.top + (this.offsetParent[0] == document.body ? 0 : this.offsetParent[0].scrollTop)}, parseInt(this.options.revert, 10) || 500, function() {
        c._clear(a);
      });
...
this.currentItem.show(); for (var b = this.containers.length - 1; b >= 0; b--) {
  this.containers[b]._trigger('deactivate', null, a._uiHash(this)); if (this.containers[b].containerCache.over) {
    this.containers[b]._trigger('out', null, a._uiHash(this)); this.containers[b].containerCache.over = 0;
  }
}
  } if (this.placeholder) {
    this.placeholder[0].parentNode && this.placeholder[0].parentNode.removeChild(this.placeholder[0]); this.options.helper != 'original' && this.helper && this.helper[0].parentNode && this.helper.remove(); d.extend(this, {helper:null,
      dragging:false, reverting:false, _noFinalSort:null}); this.domPosition.prev ? d(this.domPosition.prev).after(this.currentItem) : d(this.domPosition.parent).prepend(this.currentItem);
  } return this;
  }, serialize:function(a) {
  var b = this._getItemsAsjQuery(a && a.connected), c = []; a = a || {}; d(b).each(function() {
    var e = (d(a.item || this).attr(a.attribute || 'id') || '').match(a.expression || /(.+)[-=_](.+)/); if (e)c.push((a.key || e[1] + '[]') + '=' + (a.key && a.expression ? e[1] : e[2]));
  }); !c.length && a.key && c.push(a.key + '='); return c.join('&');
},
    toArray:function(a) {
...
this.options.tolerance != 'pointer' && this.helperProportions[this.floating ? 'width' : 'height'] > a[this.floating ? 'width' : 'height'] ? j : g < b + this.helperProportions.width / 2 && c - this.helperProportions.width / 2 < h && i < e + this.helperProportions.height / 2 && f - this.helperProportions.height / 2 < k;
    ...
    _getDragVerticalDirection:function() {
      ...
      if (e && a) for (a = e.length - 1; a >= 0; a--) for (var f = d(e[a]), g = f.length - 1; g >= 0; g--) {
        var h = d.data(f[g], 'sortable'); if (h && h != this && !h.options.disabled)c.push([d.isFunction(h.options.items) ? h.options.items.call(h.element) : d(h.options.items, h.element).not('.ui-sortable-helper').not('.ui-sortable-placeholder'), h]);
      ...
      this.items = []; this.containers = [this]; var b = this.items, c = [[d.isFunction(this.options.items) ? this.options.items.call(this.element[0], a, {item:this.currentItem}) : d(this.options.items, this.element),
        ...
        var i = d.data(g[h], 'sortable'); if (i && i != this && !i.options.disabled) {
          c.push([d.isFunction(i.options.items) ? i.options.items.call(i.element[0], a, {item:this.currentItem}) : d(i.options.items, i.element), i]); this.containers.push(i);
        }
      } for (f = c.length - 1; f >= 0; f--) {
        a = c[f][1]; e = c[f][0]; h = 0; for (g = e.length; h < g; h++) {
          i = d(e[h]); i.data('sortable-item', a); b.push({item:i, instance:a, width:0, height:0, left:0, top:0});
        }
      }
...
this.helper) this.offset.parent = this._getParentOffset(); for (var b = this.items.length - 1; b >= 0; b--) {
  var c = this.items[b], e = this.options.toleranceElement ? d(this.options.toleranceElement, c.item) : c.item; if (!a) {
    c.width = e.outerWidth(); c.height = e.outerHeight();
...
e.left; this.containers[b].containerCache.top = e.top; this.containers[b].containerCache.width = this.containers[b].element.outerWidth(); this.containers[b].containerCache.height = this.containers[b].element.outerHeight();
  ...
  for (var b = null, c = null, e = this.containers.length - 1; e >= 0; e--) if (!d.ui.contains(this.currentItem[0], this.containers[e].element[0])) if (this._intersectsWith(this.containers[e].containerCache)) {
    ...
    this.containers[e]._trigger('out', a, this._uiHash(this)); this.containers[e].containerCache.over = 0;
  } if (b) if (this.containers.length ===
1) {
    this.containers[c]._trigger('over', a, this._uiHash(this)); this.containers[c].containerCache.over = 1;
...
'' || b.forceHelperSize)a.height(this.currentItem.height()); return a;
  ...
  this.offsetParent =
this.helper.offsetParent(); var a = this.offsetParent.offset(); if (this.cssPosition == 'absolute' && this.scrollParent[0] != document && d.ui.contains(this.scrollParent[0], this.offsetParent[0])) {
  a.left += this.scrollParent.scrollLeft(); a.top += this.scrollParent.scrollTop();
} if (this.offsetParent[0] == document.body || this.offsetParent[0].tagName && this.offsetParent[0].tagName.toLowerCase() == 'html' && d.browser.msie)a = {top:0, left:0}; return {top:a.top + (parseInt(this.offsetParent.css('borderTopWidth'), 10) || 0), left:a.left + (parseInt(this.offsetParent.css('borderLeftWidth'),
10) || 0)};
}, _getRelativeOffset:function() {
  if (this.cssPosition == 'relative') {
...
this.counter ? ++this.counter : 1; var f = this, g = this.counter; window.setTimeout(function() {
...
this.currentItem[0]) && !b) {
    ...
    }.call(this, this.containers[e])); if (this.containers[e].containerCache.over) {
      c.push(function(f) {
        return function(g) {
          f._trigger('out',
g, this._uiHash(this));
    ...
    if (!b) {
      this._trigger('beforeStop', a, this._uiHash()); for (e = 0; e < c.length; e++)c[e].call(this, a); this._trigger('stop', a, this._uiHash());
    } return false;
  }b ||
this._trigger('beforeStop', a, this._uiHash()); this.placeholder[0].parentNode.removeChild(this.placeholder[0]); this.helper[0] != this.currentItem[0] && this.helper.remove(); this.helper = null; if (!b) {
  ...
  c.widget('ui.accordion', {options:{active:0, animated:'slide', autoHeight:true, clearStyle:false, collapsible:false, event:'click', fillSpace:false, header:'> li > :first-child,> :not(li):even', icons:{header:'ui-icon-triangle-1-e', headerSelected:'ui-icon-triangle-1-s'}, navigation:false, navigationFilter:function() {
    ...
    if (b.navigation) {
      var d = a.element.find('a').filter(b.navigationFilter).eq(0); if (d.length) {
        var h = d.closest('.ui-accordion-header'); a.active = h.length ? h : d.closest('.ui-accordion-content').prev();
      }
    }a.active = a._findActive(a.active || b.active).addClass('ui-state-default ui-state-active').toggleClass('ui-corner-all').toggleClass('ui-corner-top'); a.active.next().addClass('ui-accordion-content-active'); a._createIcons(); a.resize(); a.element.attr('role', 'tablist'); a.headers.attr('role', 'tab').bind('keydown.accordion',
function(f) {
  return a._keydown(f);
}).next().attr('role', 'tabpanel'); a.headers.not(a.active || '').attr({'aria-expanded':'false', 'aria-selected':'false', tabIndex:-1}).next().hide(); a.active.length ? a.active.attr({'aria-expanded':'true', 'aria-selected':'true', tabIndex:0}) : a.headers.eq(0).attr('tabIndex', 0); c.browser.safari || a.headers.find('a').attr('tabIndex', -1); b.event && a.headers.bind(b.event.split(' ').join('.accordion ') + '.accordion', function(f) {
  a._clickHandler.call(a, f, this); f.preventDefault();
});
  }, _createIcons:function() {
    var a =
this.options; if (a.icons) {
  c('<span></span>').addClass('ui-icon ' + a.icons.header).prependTo(this.headers); this.active.children('.ui-icon').toggleClass(a.icons.header).toggleClass(a.icons.headerSelected); this.element.addClass('ui-accordion-icons');
}
    ...
    if (!(this.options.disabled || a.altKey || a.ctrlKey)) {
      ...
      c(a.target).attr('tabIndex', -1); c(f).attr('tabIndex', 0); f.focus(); return false;
    } return true;
    }
  }, resize:function() {
    ...
    }b = this.element.parent().height(); c.browser.msie && this.element.parent().css('overflow', d); this.headers.each(function() {
...
c(this).height()));
  ...
  this.options.active = a; a = this._findActive(a)[0]; this._clickHandler({target:a}, a); return this;
  ...
  if (!d.disabled) if (a.target) {
    a = c(a.currentTarget || b); b = a[0] === this.active[0]; d.active = d.collapsible && b ? false : this.headers.index(a); if (!(this.running || !d.collapsible && b)) {
      ...
      if (!b) {
          a.removeClass('ui-state-default ui-corner-all').addClass('ui-state-active ui-corner-top').children('.ui-icon').removeClass(d.icons.header).addClass(d.icons.headerSelected); a.next().addClass('ui-accordion-content-active');
        }
    }
  ...
  }; g._trigger('changestart', null, g.data); g.running = b.size() === 0 ? a.size() : b.size(); if (e.animated) {
...
e.fillSpace}; if (!e.proxied)e.proxied = e.animated; if (!e.proxiedDuration)e.proxiedDuration = e.duration; e.animated = c.isFunction(e.proxied) ? e.proxied(d) : e.proxied; e.duration = c.isFunction(e.proxiedDuration) ? e.proxiedDuration(d) : e.proxiedDuration; h = c.ui.accordion.animations; var i = e.duration, k = e.animated; if (k && !h[k] && !c.easing[k])k = 'slide'; h[k] || (h[k] = function(l) {
  this.slide(l, {easing:k, duration:i || 700});
    ...
    'aria-selected':'false', tabIndex:-1}).blur(); a.prev().attr({'aria-expanded':'true', 'aria-selected':'true', tabIndex:0}).focus();
  ...
  this.running = a ? 0 : --this.running; if (!this.running) {
    ...
    a = c.extend({easing:'swing', duration:300}, a, b); if (a.toHide.size()) if (a.toShow.size()) {
      ...
      }); a.toShow.css({height:0, overflow:'hidden'}).show(); a.toHide.filter(':hidden').each(a.complete).end().filter(':visible').animate(g, {step:function(j, i) {
        if (i.prop == 'height')h = i.end - i.start === 0 ? 0 : (i.now - i.start) / (i.end - i.start); a.toShow[0].style[i.prop] = h * f[i.prop].value + f[i.prop].unit;
      }, duration:a.duration, easing:a.easing, complete:function() {
        a.autoHeight || a.toShow.css('height', ''); a.toShow.css({width:e, overflow:d}); a.complete();
      }});
...
a.element.val(f.value);
      ...
      var h = f.item.data('item.autocomplete'), i = a.previous; if (a.element[0] !== b.activeElement) {
        a.element.focus(); a.previous = i; setTimeout(function() {
          a.previous = i; a.selectedItem = h;
        }, 1);
      }false !== a._trigger('select', c, {item:h}) && a.element.val(h.value); a.term = a.element.val(); a.close(c); a.selectedItem = h;
    }, blur:function() {
...
b && this.xhr && this.xhr.abort();
    ...
    var a = this, b, g; if (d.isArray(this.options.source)) {
      ...
      g = this.options.source; this.source = function(c, f) {
        a.xhr && a.xhr.abort(); a.xhr = d.ajax({url:g, data:c, dataType:'json', autocompleteRequest:++e, success:function(h) {
...
b.value, value:b.value || b.label}, b);
  ...
  var b = this.menu.element.empty().zIndex(this.element.zIndex() + 1); this._renderMenu(b, a); this.menu.deactivate(); this.menu.refresh(); b.show(); this._resizeMenu(); b.position(d.extend({of:this.element}, this.options.position)); this.options.autoFocus && this.menu.next(new d.Event('mouseover'));
}, _resizeMenu:function() {
  var a = this.menu.element; a.outerWidth(Math.max(a.width('').outerWidth(), this.element.outerWidth()));
...
(function(d) {
  d.widget('ui.menu', {_create:function() {
    var e = this; this.element.addClass('ui-menu ui-widget ui-widget-content ui-corner-all').attr({role:'listbox', 'aria-activedescendant':'ui-active-menuitem'}).click(function(a) {
      if (d(a.target).closest('.ui-menu-item a').length) {
        a.preventDefault(); e.select(a);
      }
    }); this.refresh();
  }, refresh:function() {
...
-1).mouseenter(function(a) {
  ...
  e.deactivate();
    ...
    this.deactivate(); if (this.hasScroll()) {
      var b = a.offset().top - this.element.offset().top, g = this.element.attr('scrollTop'), c = this.element.height(); if (b < 0) this.element.attr('scrollTop', g + b); else b >= c && this.element.attr('scrollTop', g + b - c + a.height());
    } this.active = a.eq(0).children('a').addClass('ui-state-hover').attr('id', 'ui-active-menuitem').end(); this._trigger('focus', e, {item:a});
  },
    deactivate:function() {
      if (this.active) {
      ...
      var a = this.active.offset().top, b = this.element.height(), g = this.element.children('.ui-menu-item').filter(function() {
          var c = d(this).offset().top - a - b + d(this).height(); return c < 10 && c > -10;
        }); g.length || (g = this.element.children('.ui-menu-item:last')); this.activate(e,
g);
    ...
    if (this.hasScroll()) if (!this.active || this.first()) this.activate(e, this.element.children('.ui-menu-item:last')); else {
      ...
      var g = d(this).offset().top - a + b - d(this).height(); return g < 10 && g > -10;
...
function() {
  if (!c.disabled) {
    a(this).addClass('ui-state-hover'); this === g && a(this).addClass('ui-state-active');
  }
  ...
  this.buttonElement.bind('mousedown.button', function() {
    ...
    if (c.disabled) return false; if (e.keyCode == a.ui.keyCode.SPACE || e.keyCode == a.ui.keyCode.ENTER)a(this).addClass('ui-state-active');
...
c.disabled);
      ...
      this.type = this.element.is(':checkbox') ? 'checkbox' : this.element.is(':radio') ? 'radio' : this.element.is('input') ? 'input' : 'button'; if (this.type === 'checkbox' || this.type === 'radio') {
        var b = this.element.parents().filter(':last'), c = 'label[for=' + this.element.attr('id') + ']'; this.buttonElement = b.find(c); if (!this.buttonElement.length) {
          b = b.length ? b.siblings() : this.element.siblings(); this.buttonElement = b.filter(c); if (!this.buttonElement.length) this.buttonElement = b.find(c);
      ...
      a.Widget.prototype._setOption.apply(this, arguments); if (b === 'disabled')c ? this.element.attr('disabled', true) : this.element.removeAttr('disabled'); this._resetButton();
    }, refresh:function() {
...
-1).css('outline', 0).keydown(function(i) {
  if (b.closeOnEscape && i.keyCode && i.keyCode === c.ui.keyCode.ESCAPE) {
    a.close(i); i.preventDefault();
  }
}).attr({role:'dialog', 'aria-labelledby':e}).mousedown(function(i) {
  a.moveToTop(false, i);
}); a.element.show().removeAttr('title').addClass('ui-dialog-content ui-widget-content').appendTo(g); var f = (a.uiDialogTitlebar = c('<div></div>')).addClass('ui-dialog-titlebar ui-widget-header ui-corner-all ui-helper-clearfix').prependTo(g), h = c('<a href="#"></a>').addClass('ui-dialog-titlebar-close ui-corner-all').attr('role',
...
a.element.attr('title', a.originalTitle); return a;
    ...
    var b = this, d, e; if (false !== b._trigger('beforeClose', a)) {
      b.overlay && b.overlay.destroy(); b.uiDialog.unbind('keypress.ui-dialog'); b._isOpen = false; if (b.options.hide)b.uiDialog.hide(b.options.hide, function() {
        ...
        b.uiDialog.hide(); b._trigger('close', a);
      }c.ui.dialog.overlay.resize(); if (b.options.modal) {
  ...
  }a = {scrollTop:d.element.attr('scrollTop'), scrollLeft:d.element.attr('scrollLeft')}; c.ui.dialog.maxZ += 1; d.uiDialog.css('z-index', c.ui.dialog.maxZ);
    ...
    var a = this, b = a.options, d = a.uiDialog; a.overlay = b.modal ? new c.ui.dialog.overlay(a) : null; a._size(); a._position(b.position); d.show(b.show); a.moveToTop(true); b.modal && d.bind('keypress.ui-dialog', function(e) {
      if (e.keyCode === c.ui.keyCode.TAB) {
        var g = c(':tabbable', this), f = g.filter(':first'); g = g.filter(':last'); if (e.target === g[0] && !e.shiftKey) {
          f.focus(1); return false;
        } else if (e.target === f[0] && e.shiftKey) {
          g.focus(1); return false;
        }
      }
    });
  ...
  var b = this, d = false, e = c('<div></div>').addClass('ui-dialog-buttonpane ui-widget-content ui-helper-clearfix'), g = c('<div></div>').addClass('ui-dialog-buttonset').appendTo(e); b.uiDialog.find('.ui-dialog-buttonpane').remove(); typeof a === 'object' && a !== null && c.each(a, function() {
    return !(d = true);
  }); if (d) {
    c.each(a, function(f,
h) {
  ...
  } var b = this, d = b.options, e = c(document), g; b.uiDialog.draggable({cancel:'.ui-dialog-content, .ui-dialog-titlebar-close', handle:'.ui-dialog-titlebar', containment:'document', start:function(f, h) {
    g =
d.height === 'auto' ? 'auto' : c(this).height(); c(this).height(c(this).height()).addClass('ui-dialog-dragging'); b._trigger('dragStart', f, a(h));
  }, drag:function(f, h) {
    b._trigger('drag', f, a(h));
  }, stop:function(f, h) {
    d.position = [h.position.left - e.scrollLeft(), h.position.top - e.scrollTop()]; c(this).removeClass('ui-dialog-dragging').height(g); b._trigger('dragStop', f, a(h)); c.ui.dialog.overlay.resize();
  }});
}, _makeResizable:function(a) {
  function b(f) {
    return {originalPosition:f.originalPosition, originalSize:f.originalSize,
      position:f.position, size:f.size};
  }a = a === j ? this.options.resizable : a; var d = this, e = d.options, g = d.uiDialog.css('position'); a = typeof a === 'string' ? a : 'n,e,s,w,se,sw,ne,nw'; d.uiDialog.resizable({cancel:'.ui-dialog-content', containment:'document', alsoResize:d.element, maxWidth:e.maxWidth, maxHeight:e.maxHeight, minWidth:e.minWidth, minHeight:d._minHeight(), handles:a, start:function(f, h) {
    c(this).addClass('ui-dialog-resizing'); d._trigger('resizeStart', f, b(h));
  }, resize:function(f, h) {
    d._trigger('resize', f, b(h));
  }, stop:function(f,
h) {
    c(this).removeClass('ui-dialog-resizing'); e.height = c(this).height(); e.width = c(this).width(); d._trigger('resizeStop', f, b(h)); c.ui.dialog.overlay.resize();
...
1)b[1] = b[0]; c.each(['left', 'top'], function(g, f) {
  if (+b[g] === b[g]) {
    d[g] = b[g]; b[g] = f;
  }
...
this.uiDialog.is(':visible'); this.element.show().css({width:'auto', minHeight:0, height:0}); if (a.minWidth > a.width)a.width = a.minWidth; b = this.uiDialog.css({height:'auto', width:a.width}).height(); d = Math.max(0, a.minHeight - b); if (a.height === 'auto') if (c.support.minHeight) this.element.css({minHeight:d, height:'auto'}); else {
  this.uiDialog.show(); a = this.element.css('height', 'auto').height(); e || this.uiDialog.hide(); this.element.height(Math.max(a, d));
} else this.element.height(Math.max(a.height - b, 0)); this.uiDialog.is(':data(resizable)') &&
...
0) {
  ...
  var a, b; if (c.browser.msie && c.browser.version < 7) {
    a = Math.max(document.documentElement.scrollWidth, document.body.scrollWidth); b = Math.max(document.documentElement.offsetWidth, document.body.offsetWidth); return a < b ? c(window).width() + 'px' : a + 'px';
...
(function(d) {
  d.widget('ui.slider', d.ui.mouse, {widgetEventPrefix:'slide', options:{animate:false, distance:0, max:100, min:0, orientation:'horizontal', range:false, step:1, value:0, values:null}, _create:function() {
    ...
    this.range = d([]); if (a.range) {
      if (a.range === true) {
        this.range = d('<div></div>'); if (!a.values)a.values = [this._valueMin(), this._valueMin()]; if (a.values.length && a.values.length !== 2)a.values = [a.values[0], a.values[0]];
      } else this.range = d('<div></div>'); this.range.appendTo(this.element).addClass('ui-slider-range'); if (a.range === 'min' || a.range === 'max') this.range.addClass('ui-slider-range-' + a.range); this.range.addClass('ui-widget-header');
    ...
    if (a.values && a.values.length) for (;d('.ui-slider-handle', this.element).length < a.values.length;)d("<a href='#'></a>").appendTo(this.element).addClass('ui-slider-handle'); this.handles = d('.ui-slider-handle', this.element).addClass('ui-state-default ui-corner-all'); this.handle = this.handles.eq(0); this.handles.add(this.range).filter('a').click(function(c) {
      ...
      if (a.disabled)d(this).blur();
        ...
        d('.ui-slider .ui-state-focus').removeClass('ui-state-focus'); d(this).addClass('ui-state-focus');
      }
    }).blur(function() {
...
false; if (!b._keySliding) {
  b._keySliding = true; d(this).addClass('ui-state-active'); h = b._start(c, f); if (h === false) return;
} break;
...
b._valueMax()) return; g = b._trimAlignValue(h + i); break; case d.ui.keyCode.DOWN:case d.ui.keyCode.LEFT:if (h === b._valueMin()) return; g = b._trimAlignValue(h - i); break;
    ...
    var e = d(this).data('index.ui-slider-handle'); if (b._keySliding) {
      b._keySliding = false; b._stop(c, e); b._change(c, e); d(this).removeClass('ui-state-active');
    }
    ...
    var a = this.options, c, e, f, h, g; if (a.disabled) return false; this.elementSize = {width:this.element.outerWidth(), height:this.element.outerHeight()}; this.elementOffset = this.element.offset(); c = this._normValueFromMouse({x:b.pageX, y:b.pageY}); e = this._valueMax() - this._valueMin() + 1; h = this; this.handles.each(function(i) {
      var j = Math.abs(c - h.values(i)); if (e > j) {
        e = j; f = d(this); g = i;
      }
...
g) === false) return false; this._mouseSliding = true; h._handleIndex = g; f.addClass('ui-state-active').focus(); a = f.offset(); this._clickOffset = !d(b.target).parents().andSelf().is('.ui-slider-handle') ? {left:0, top:0} : {left:b.pageX - a.left - f.width() / 2, top:b.pageY - a.top - f.height() / 2 - (parseInt(f.css('borderTopWidth'), 10) || 0) - (parseInt(f.css('borderBottomWidth'), 10) || 0) + (parseInt(f.css('marginTop'), 10) || 0)}; this.handles.hasClass('ui-state-hover') || this._slide(b, g, c); return this._animateOff = true;
    ...
    _mouseDrag:function(b) {
      var a = this._normValueFromMouse({x:b.pageX, y:b.pageY}); this._slide(b, this._handleIndex, a); return false;
    }, _mouseStop:function(b) {
      this.handles.removeClass('ui-state-active'); this._mouseSliding = false; this._stop(b, this._handleIndex); this._change(b, this._handleIndex); this._clickOffset = this._handleIndex = null; return this._animateOff = false;
    }, _detectOrientation:function() {
      this.orientation = this.options.orientation === 'vertical' ? 'vertical' : 'horizontal';
    }, _normValueFromMouse:function(b) {
      var a;
      if (this.orientation === 'horizontal') {
        a = this.elementSize.width; b = b.x - this.elementOffset.left - (this._clickOffset ? this._clickOffset.left : 0);
      } else {
        a = this.elementSize.height; b = b.y - this.elementOffset.top - (this._clickOffset ? this._clickOffset.top : 0);
      }a = b / a; if (a > 1)a = 1; if (a < 0)a = 0; if (this.orientation === 'vertical')a = 1 - a; b = this._valueMax() - this._valueMin(); return this._trimAlignValue(this._valueMin() + a * b);
    }, _start:function(b, a) {
...
this.values(a); c.values = this.values();
      ...
      var e; if (this.options.values && this.options.values.length) {
        e = this.values(a ? 0 : 1); if (this.options.values.length === 2 && this.options.range === true && (a === 0 && c > e || a === 1 && c < e))c = e; if (c !== this.values(a)) {
        e = this.values(); e[a] = c; b = this._trigger('slide', b, {handle:this.handles[a], value:c, values:e}); this.values(a ? 0 : 1); b !== false && this.values(a, c, true);
      }
      } else if (c !== this.value()) {
      b = this._trigger('slide', b, {handle:this.handles[a],
      value:c}); b !== false && this.value(c);
    }
    }, _stop:function(b, a) {
      var c = {handle:this.handles[a], value:this.value()}; if (this.options.values && this.options.values.length) {
...
this._trimAlignValue(b); this._refreshValue(); this._change(null, 0);
  ...
  var c, e, f; if (arguments.length > 1) {
    this.options.values[b] = this._trimAlignValue(a); this._refreshValue(); this._change(null, b);
  } if (arguments.length) if (d.isArray(arguments[0])) {
    c = this.options.values; e = arguments[0]; for (f = 0; f < c.length; f += 1) {
      c[f] = this._trimAlignValue(e[f]); this._change(null, f);
    } this._refreshValue();
  ...
  var b = this.options.value; return b = this._trimAlignValue(b);
}, _values:function(b) {
  var a, c; if (arguments.length) {
    a = this.options.values[b];
    return a = this._trimAlignValue(a);
  } else {
    a = this.options.values.slice(); for (c = 0; c < a.length; c += 1)a[c] = this._trimAlignValue(a[c]); return a;
  }
}, _trimAlignValue:function(b) {
  if (b <= this._valueMin()) return this._valueMin(); if (b >= this._valueMax()) return this._valueMax(); var a = this.options.step > 0 ? this.options.step : 1, c = (b - this._valueMin()) % a; alignValue = b - c; if (Math.abs(c) * 2 >= a)alignValue += c > 0 ? a : -a; return parseFloat(alignValue.toFixed(5));
}, _valueMin:function() {
    ...
    _refreshValue:function() {
      var b = this.options.range, a = this.options, c = this, e = !this._animateOff ? a.animate : false, f, h = {}, g, i, j, l; if (this.options.values && this.options.values.length) this.handles.each(function(k) {
        f = (c.values(k) - c._valueMin()) / (c._valueMax() - c._valueMin()) * 100; h[c.orientation === 'horizontal' ? 'left' : 'bottom'] = f + '%'; d(this).stop(1, 1)[e ? 'animate' : 'css'](h, a.animate); if (c.options.range === true) if (c.orientation === 'horizontal') {
          ...
          if (k === 0)c.range.stop(1, 1)[e ? 'animate' : 'css']({bottom:f + '%'}, a.animate); if (k === 1)c.range[e ? 'animate' : 'css']({height:f - g + '%'}, {queue:false, duration:a.animate});
        }g = f;
      }); else {
...
e == this.options.selected || this.select(e); else {
    ...
    var b = this.cookie || (this.cookie = this.options.cookie.name || 'ui-tabs-' + w()); return d.cookie.apply(null, [b].concat(d.makeArray(arguments)));
  }, _ui:function(b, e) {
...
d(this); b.html(b.data('label.tabs')).removeData('label.tabs');
  ...
  } var a = this, c = this.options, h = /^#.+/; this.list = this.element.find('ol,ul').eq(0); this.lis = d(' > li:has(a[href])', this.list); this.anchors = this.lis.map(function() {
    return d('a', this)[0];
  }); this.panels = d([]); this.anchors.each(function(g, f) {
...
(q = d('base')[0]) && l === q.href)) {
      ...
      d.data(f, 'href.tabs', i); d.data(f, 'load.tabs', i.replace(/#.*$/, '')); i = a._tabId(f); f.href = '#' + i; f = a.element.find('#' + i); if (!f.length) {
        f = d(c.panelTemplate).attr('id', i).addClass('ui-tabs-panel ui-widget-content ui-corner-bottom').insertAfter(a.panels[g - 1] || a.list); f.data('destroy.tabs', true);
    ...
    this.list.addClass('ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-widget-header ui-corner-all'); this.lis.addClass('ui-state-default ui-corner-top'); this.panels.addClass('ui-tabs-panel ui-widget-content ui-corner-bottom'); if (c.selected === p) {
      ...
      }); if (typeof c.selected !== 'number' && c.cookie)c.selected = parseInt(a._cookie(), 10); if (typeof c.selected !== 'number' && this.lis.filter('.ui-tabs-selected').length)c.selected =
this.lis.index(this.lis.filter('.ui-tabs-selected')); c.selected = c.selected || (this.lis.length ? 0 : -1);
    ...
    }))).sort(); d.inArray(c.selected, c.disabled) != -1 && c.disabled.splice(d.inArray(c.selected, c.disabled), 1); this.panels.addClass('ui-tabs-hide'); this.lis.removeClass('ui-tabs-selected ui-state-active');
...
function() {
  e(f, o); a._trigger('show', null, a._ui(g, f[0]));
});
    } : function(g, f) {
      d(g).closest('li').addClass('ui-tabs-selected ui-state-active'); f.removeClass('ui-tabs-hide'); a._trigger('show', null, a._ui(g, f[0]));
    }, s = m ? function(g, f) {
      f.animate(m, m.duration || 'normal', function() {
        a.lis.removeClass('ui-tabs-selected ui-state-active'); f.addClass('ui-tabs-hide'); e(f, m); a.element.dequeue('tabs');
      });
    } : function(g, f) {
  ...
  this.anchors.bind(c.event + '.tabs', function() {
    ...
    }c.selected = a.anchors.index(this); a.abort(); if (c.collapsible) if (f.hasClass('ui-tabs-selected')) {
      c.selected =
-1; c.cookie && a._cookie(c.selected, c.cookie); a.element.queue('tabs', function() {
  s(g, i);
      ...
      c.cookie && a._cookie(c.selected, c.cookie); a.element.queue('tabs', function() {
        r(g, l);
      }); a.load(a.anchors.index(this)); this.blur(); return false;
    }c.cookie && a._cookie(c.selected, c.cookie); if (l.length) {
  ...
  if (typeof b == 'string')b = this.anchors.index(this.anchors.filter('[href$=' + b + ']')); return b;
}, destroy:function() {
...
d.data(this, 'href.tabs'); if (e) this.href = e; var a = d(this).unbind('.tabs'); d.each(['href', 'load', 'cache'], function(c, h) {
  ...
  }); b.cookie && this._cookie(null, b.cookie); return this;
}, add:function(b,
e, a) {
  if (a === p)a = this.anchors.length; var c = this, h = this.options; e = d(h.tabTemplate.replace(/#\{href\}/g, b).replace(/#\{label\}/g, e)); b = !b.indexOf('#') ? b.replace('#', '') : this._tabId(d('a', e)[0]); e.addClass('ui-state-default ui-corner-top').data('destroy.tabs', true); var j = c.element.find('#' + b); j.length || (j = d(h.panelTemplate).attr('id', b).data('destroy.tabs', true)); j.addClass('ui-tabs-panel ui-widget-content ui-corner-bottom ui-tabs-hide'); if (a >= this.lis.length) {
    e.appendTo(this.list); j.appendTo(this.list[0].parentNode);
  } else {
  ...
  if (a.hasClass('ui-tabs-selected') && this.anchors.length > 1) this.select(b + (b + 1 < this.anchors.length ? 1 : -1)); e.disabled = d.map(d.grep(e.disabled, function(h) {
    ...
    return h >= b ? --h : h;
  }); this._tabify(); this._trigger('remove', null, this._ui(a.find('a')[0], c[0])); return this;
}, enable:function(b) {
...
this._ui(this.anchors[b], this.panels[b])); return this;
  ...
  b = this._getIndex(b); var e = this.options; if (b != e.selected) {
    this.lis.eq(b).addClass('ui-state-disabled'); e.disabled.push(b); e.disabled.sort(); this._trigger('disable', null, this._ui(this.anchors[b], this.panels[b]));
  } return this;
}, select:function(b) {
    ...
    load:function(b) {
      b = this._getIndex(b); var e = this, a = this.options, c = this.anchors.eq(b)[0], h = d.data(c, 'load.tabs'); this.abort(); if (!h || this.element.queue('tabs').length !== 0 && d.data(c, 'cache.tabs')) this.element.dequeue('tabs'); else {
        this.lis.eq(b).addClass('ui-state-processing'); if (a.spinner) {
          var j = d('span', c); j.data('label.tabs', j.html()).html(a.spinner);
...
'cache.tabs', true); e._trigger('load', null, e._ui(e.anchors[b], e.panels[b])); try {
  a.ajaxOptions.success(k, n);
    ...
    url:function(b, e) {
      this.anchors.eq(b).removeData('cache.tabs').data('load.tabs', e); return this;
    }, length:function() {
      ...
      var a = this, c = this.options, h = a._rotate || (a._rotate = function(j) {
          clearTimeout(a.rotation); a.rotation = setTimeout(function() {
            var k = c.selected; a.select(++k < a.anchors.length ? k : 0);
          }, b); j && j.stopPropagation();
...
a.rotate(null);
        ...
        this.element.bind('tabsshow', h); this.anchors.bind(c.event + '.tabs', e); h();
      } else {
        clearTimeout(a.rotation); this.element.unbind('tabsshow', h); this.anchors.unbind(c.event + '.tabs', e); delete this._rotate; delete this._unrotate;
      } return this;
    }});
})(jQuery);
;/*
...
null || b[c] == A)a[c] = b[c]; return a;
  }d.extend(d.ui, {datepicker:{version:'1.8.11'}}); var y = (new Date).getTime(); d.extend(K.prototype, {markerClassName:'hasDatepicker', log:function() {
    this.debug && console.log.apply('', arguments);
  }, _widgetDatepicker:function() {
    return this.dpDiv;
  }, setDefaults:function(a) {
    F(this._defaults, a || {}); return this;
    ...
    } var i = this._newInst(d(a), f); i.settings = d.extend({}, b || {}, c || {}); if (e == 'input') this._connectDatepicker(a, i); else f && this._inlineDatepicker(a, i);
    ...
    _connectDatepicker:function(a, b) {
      var c = d(a); b.append = d([]); b.trigger = d([]); if (!c.hasClass(this.markerClassName)) {
        this._attachments(c, b); c.addClass(this.markerClassName).keydown(this._doKeyDown).keypress(this._doKeyPress).keyup(this._doKeyUp).bind('setData.datepicker', function(e, f, h) {
          b.settings[f] = h;
...
b.append.remove(); if (c) {
  b.append = d('<span class="' + this._appendClass + '">' + c + '</span>'); a[e ? 'before' : 'after'](b.append);
}a.unbind('focus', this._showDatepicker); b.trigger && b.trigger.remove(); c = this._get(b, 'showOn'); if (c == 'focus' || c == 'both')a.focus(this._showDatepicker); if (c == 'button' || c == 'both') {
...
'' ? c : d('<img/>').attr({src:f, alt:c, title:c}))); a[e ? 'before' : 'after'](b.trigger); b.trigger.click(function() {
  d.datepicker._datepickerShowing && d.datepicker._lastInput == a[0] ? d.datepicker._hideDatepicker() : d.datepicker._showDatepicker(a[0]); return false;
});
}
...
c.match(/MM/) ? 'monthNames' : 'monthNamesShort'))); b.setDate(e(this._get(a, c.match(/DD/) ? 'dayNames' : 'dayNamesShort')) + 20 - b.getDay());
      ...
      var c = d(a); if (!c.hasClass(this.markerClassName)) {
        ...
        return this._get(b, f);
      }); d.data(a, 'datepicker', b); this._setDate(b, this._getDefaultDate(b),
true); this._updateDatepicker(b); this._updateAlternate(b); b.dpDiv.show();
      }
    }, _dialogDatepicker:function(a, b, c, e, f) {
      a = this._dialogInst; if (!a) {
      this.uuid += 1; this._dialogInput = d('<input type="text" id="' + ('dp' + this.uuid) + '" style="position: absolute; top: -100px; width: 0px; z-index: -10;"/>'); this._dialogInput.keydown(this._doKeyDown); d('body').append(this._dialogInput); a = this._dialogInst = this._newInst(this._dialogInput, false); a.settings = {}; d.data(this._dialogInput[0], 'datepicker', a);
    }F(a.settings, e || {});
...
this._doKeyUp);
    ...
    var b = d(a), c = d.data(a, 'datepicker'); if (b.hasClass(this.markerClassName)) {
      ...
      } else if (e == 'div' || e == 'span')b.children('.' + this._inlineClass).children().removeClass('ui-state-disabled'); this._disabledInputs = d.map(this._disabledInputs,
function(f) {
  return f == a ? null : f;
});
    }
    ...
    } else if (e == 'div' || e == 'span')b.children('.' + this._inlineClass).children().addClass('ui-state-disabled'); this._disabledInputs = d.map(this._disabledInputs, function(f) {
      return f == a ? null :
f;
    }); this._disabledInputs[this._disabledInputs.length] = a;
    }
  }, _isDisabledDatepicker:function(a) {
    if (!a) return false; for (var b = 0; b < this._disabledInputs.length; b++) if (this._disabledInputs[b] == a) return true; return false;
  }, _getInst:function(a) {
  try {
...
e.settings) : this._get(e, b) : null; var f = b || {}; if (typeof b == 'string') {
  f = {}; f[b] = c;
} if (e) {
  this._curInst == e && this._hideDatepicker(); var h = this._getDateDatepicker(a, true), i = this._getMinMaxDate(e, 'min'), g = this._getMinMaxDate(e, 'max'); F(e.settings, f); if (i !== null && f.dateFormat !== A && f.minDate === A)e.settings.minDate = this._formatDate(e, i); if (g !== null && f.dateFormat !== A && f.maxDate === A)e.settings.maxDate = this._formatDate(e, g); this._attachments(d(a), e); this._autoSize(e); this._setDateDatepicker(a, h); this._updateDatepicker(e);
}
},
    _changeDatepicker:function(a, b, c) {
      ...
      if (a = this._getInst(a)) {
        this._setDate(a, b); this._updateDatepicker(a); this._updateAlternate(a);
      }
...
+7, 'D'); c = a.ctrlKey || a.metaKey; break; default:c = false;
    ...
    var b = d.datepicker._getInst(a.target); if (d.datepicker._get(b, 'constrainInput')) {
      b = d.datepicker._possibleChars(d.datepicker._get(b, 'dateFormat')); var c = String.fromCharCode(a.charCode == A ? a.keyCode : a.charCode); return a.ctrlKey || a.metaKey || c < ' ' || !b || b.indexOf(c) > -1;
    }
  }, _doKeyUp:function(a) {
    a = d.datepicker._getInst(a.target);
    if (a.input.val() != a.lastVal) try {
      if (d.datepicker.parseDate(d.datepicker._get(a, 'dateFormat'), a.input ? a.input.val() : null, d.datepicker._getFormatConfig(a))) {
      d.datepicker._setDateFromField(a); d.datepicker._updateAlternate(a); d.datepicker._updateDatepicker(a);
    }
    } catch (b) {
      d.datepicker.log(b);
    } return true;
    ...
    d.datepicker._curInst && d.datepicker._curInst != b && d.datepicker._curInst.dpDiv.stop(true, true); var c = d.datepicker._get(b, 'beforeShow'); F(b.settings, c ? c.apply(a, [a, b]) : {}); b.lastVal = null; d.datepicker._lastInput = a; d.datepicker._setDateFromField(b); if (d.datepicker._inDialog)a.value = ''; if (!d.datepicker._pos) {
      d.datepicker._pos = d.datepicker._findPos(a); d.datepicker._pos[1] += a.offsetHeight;
...
document.documentElement.scrollLeft; d.datepicker._pos[1] -= document.documentElement.scrollTop;
      ...
      var f = d.datepicker._get(b, 'duration'), h = function() {
          d.datepicker._datepickerShowing = true; var i = b.dpDiv.find('iframe.ui-datepicker-cover'); if (i.length) {
            var g = d.datepicker._getBorders(b.dpDiv); i.css({left:-g[0], top:-g[1], width:b.dpDiv.outerWidth(), height:b.dpDiv.outerHeight()});
          }
        }; b.dpDiv.zIndex(d(a).zIndex() + 1); d.effects && d.effects[c] ? b.dpDiv.show(c, d.datepicker._get(b, 'showOptions'), f, h) : b.dpDiv[c || 'show'](c ? f : null, h); if (!c || !f)h(); b.input.is(':visible') && !b.input.is(':disabled') && b.input.focus(); d.datepicker._curInst =
b;
    }
  }
  ...
  var b = this, c = d.datepicker._getBorders(a.dpDiv); a.dpDiv.empty().append(this._generateHTML(a)); var e = a.dpDiv.find('iframe.ui-datepicker-cover'); e.length && e.css({left:-c[0], top:-c[1], width:a.dpDiv.outerWidth(), height:a.dpDiv.outerHeight()}); a.dpDiv.find('button, .ui-datepicker-prev, .ui-datepicker-next, .ui-datepicker-calendar td a').bind('mouseout', function() {
    ...
    if (!b._isDisabledDatepicker(a.inline ? a.dpDiv.parent()[0] : a.input[0])) {
      d(this).parents('.ui-datepicker-calendar').find('a').removeClass('ui-state-hover'); d(this).addClass('ui-state-hover'); this.className.indexOf('ui-datepicker-prev') != -1 && d(this).addClass('ui-datepicker-prev-hover'); this.className.indexOf('ui-datepicker-next') != -1 && d(this).addClass('ui-datepicker-next-hover');
    }
  }).end().find('.' +
...
'fadeOut' : 'hide'](a ? c : null, e); a || e(); if (a = this._get(b, 'onClose'))a.apply(b.input ? b.input[0] : null, [b.input ? b.input.val() : '', b]); this._datepickerShowing = false; this._lastInput = null; if (this._inDialog) {
  this._dialogInput.css({position:'absolute', left:'0', top:'-100px'}); if (d.blockUI) {
    d.unblockUI(); d('body').append(this.dpDiv);
  }
...
d(a.target); a[0].id != d.datepicker._mainDivId && a.parents('#' + d.datepicker._mainDivId).length == 0 && !a.hasClass(d.datepicker.markerClassName) && !a.hasClass(d.datepicker._triggerClass) && d.datepicker._datepickerShowing && !(d.datepicker._inDialog && d.blockUI) && d.datepicker._hideDatepicker();
  ...
  a =
d(a); var b = this._getInst(a[0]); if (this._get(b, 'gotoCurrent') && b.currentDay) {
  b.selectedDay = b.currentDay; b.drawMonth = b.selectedMonth = b.currentMonth; b.drawYear = b.selectedYear = b.currentYear;
} else {
  var c = new Date; b.selectedDay = c.getDate(); b.drawMonth = b.selectedMonth = c.getMonth(); b.drawYear = b.selectedYear = c.getFullYear();
...
c.monthNames : null) || this._defaults.monthNames, j = c = -1, l = -1, u = -1, k = false, o = function(p) {
  ...
  var v = o(p); p = new RegExp('^\\d{1,' + (p == '@' ? 14 : p == '!' ? 20 : p == 'y' && v ? 4 : p == 'o' ? 3 : 2) + '}'); p = b.substring(s).match(p); if (!p) throw 'Missing number at position ' + s; s += p[0].length; return parseInt(p[0], 10);
}, n = function(p, v, H) {
  p = o(p) ? H : v; for (v = 0; v < p.length; v++) if (b.substr(s, p[v].length).toLowerCase() == p[v].toLowerCase()) {
    s += p[v].length; return v + 1;
...
1; l = w.getDate(); break; case "'":if (o("'"))r(); else k = true; break; default:r();
  ...
  j = 1; l = u; do {
    e = this._getDaysInMonth(c, j - 1); if (l <= e) break; j++; l -= e;
  } while (1);
}w = this._daylightSavingAdjust(new Date(c, j - 1, l)); if (w.getFullYear() != c || w.getMonth() + 1 != j || w.getDate() != l) throw 'Invalid date'; return w;
...
a.charAt(k + 1) == o) && k++; return o;
          ...
          return i(o) ? r[m] : n[m];
        }, l = '', u = false; if (b) for (var k = 0; k < a.length; k++) if (u) if (a.charAt(k) == "'" && !i("'"))u = false; else l += a.charAt(k); else switch (a.charAt(k)) {
...
f.getMonth(); a.drawYear = a.selectedYear = f.getFullYear(); a.currentDay = e ? f.getDate() : 0; a.currentMonth = e ? f.getMonth() : 0; a.currentYear = e ? f.getFullYear() : 0; this._adjustInstDate(a);
    ...
    var e = function(h) {
        var i = new Date; i.setDate(i.getDate() + h); return i;
      }, f = function(h) {
        try {
          return d.datepicker.parseDate(d.datepicker._get(a, 'dateFormat'), h, d.datepicker._getFormatConfig(a));
        } catch (i) {} var g =
(h.toLowerCase().match(/^c/) ? d.datepicker._getDate(a) : null) || new Date, j = g.getFullYear(), l = g.getMonth(); g = g.getDate(); for (var u = /([+-]?[0-9]+)\s*(d|D|w|W|m|M|y|Y)?/g, k = u.exec(h); k;) {
  switch (k[2] || 'd') {
...
b.getMonth(), b.getDate())); var c = this._get(a, 'isRTL'), e = this._get(a, 'showButtonPanel'), f = this._get(a, 'hideIfNoPrevNext'), h = this._get(a, 'navigationAsDateFormat'), i = this._getNumberOfMonths(a), g = this._get(a, 'showCurrentAtPos'), j = this._get(a, 'stepMonths'), l = i[0] != 1 || i[1] != 1, u = this._daylightSavingAdjust(!a.currentDay ? new Date(9999, 9, 9) : new Date(a.currentYear, a.currentMonth, a.currentDay)), k = this._getMinMaxDate(a, 'min'), o = this._getMinMaxDate(a, 'max'); g = a.drawMonth - g; var m = a.drawYear; if (g < 0) {
  g += 12; m--;
} if (o) {
  var n =
this._daylightSavingAdjust(new Date(o.getFullYear(), o.getMonth() - i[0] * i[1] + 1, o.getDate())); for (n = k && n < k ? k : n; this._daylightSavingAdjust(new Date(m, g, 1)) > n;) {
  g--; if (g < 0) {
    g = 11; m--;
  }
}
...
P + '</tr>';
      ...
      g = 0; m++;
    }x += '</tbody></table>' + (l ? '</div>' + (i[0] > 0 && E == i[1] - 1 ? '<div class="ui-datepicker-row-break"></div>' : '') : ''); M += x;
  }I += M;
  ...
  var j = this._get(a, 'changeMonth'), l = this._get(a, 'changeYear'), u = this._get(a, 'showMonthAfterYear'), k = '<div class="ui-datepicker-title">',
    o = ''; if (h || !j)o += '<span class="ui-datepicker-month">' + i[b] + '</span>'; else {
...
l) ? '&#xa0;' : '')); a.yearshtml = ''; if (h || !l)k += '<span class="ui-datepicker-year">' + c + '</span>'; else {
  g = this._get(a, 'yearRange').split(':'); var r = (new Date).getFullYear(); i = function(s) {
    s = s.match(/c[+-].*/) ? c + parseInt(s.substring(1), 10) : s.match(/[+-].*/) ? r + parseInt(s, 10) : parseInt(s, 10); return isNaN(s) ? r : s;
  }; b = i(g[0]); g = Math.max(b, i(g[1] || '')); b = e ? Math.max(b, e.getFullYear()) : b; g = f ? Math.min(g, f.getFullYear()) : g; for (a.yearshtml += '<select class="ui-datepicker-year" onchange="DP_jQuery_' + y + ".datepicker._selectMonthYear('#" +
a.id + "', this, 'Y');\" onclick=\"DP_jQuery_" + y + ".datepicker._clickMonthYear('#" + a.id + "');\">"; b <= g; b++)a.yearshtml += '<option value="' + b + '"' + (b == c ? ' selected="selected"' : '') + '>' + b + '</option>'; a.yearshtml += '</select>'; if (d.browser.mozilla)k += '<select class="ui-datepicker-year"><option value="' + c + '" selected="selected">' + c + '</option></select>'; else {
  k += a.yearshtml; a.yearshtml = null;
}
...
a.drawYear + (c == 'Y' ? b : 0), f = a.drawMonth + (c == 'M' ? b : 0); b = Math.min(a.selectedDay, this._getDaysInMonth(e, f)) + (c == 'D' ? b : 0); e = this._restrictMinMax(a, this._daylightSavingAdjust(new Date(e, f, b))); a.selectedDay = e.getDate(); a.drawMonth = a.selectedMonth = e.getMonth(); a.drawYear = a.selectedYear = e.getFullYear(); if (c == 'M' || c == 'Y') this._notifyChange(a);
  ...
  var c = this._getMinMaxDate(a, 'min'); a = this._getMinMaxDate(a, 'max'); b = c && b < c ? c : b; return b = a && b > a ? a : b;
...
'onChangeMonthYear'); if (b)b.apply(a.input ? a.input[0] : null, [a.selectedYear, a.selectedMonth + 1, a]);
  ...
  var c = this._getMinMaxDate(a, 'min'); a = this._getMinMaxDate(a, 'max'); return (!c || b.getTime() >= c.getTime()) && (!a || b.getTime() <= a.getTime());
...
function(a) {
  if (!this.length) return this; if (!d.datepicker.initialized) {
    d(document).mousedown(d.datepicker._checkExternalClick).find('body').append(d.datepicker.dpDiv); d.datepicker.initialized = true;
  } var b = Array.prototype.slice.call(arguments, 1); if (typeof a == 'string' && (a == 'isDisabled' || a == 'getDate' || a == 'widget')) return d.datepicker['_' + a + 'Datepicker'].apply(d.datepicker, [this[0]].concat(b)); if (a == 'option' && arguments.length == 2 && typeof arguments[1] == 'string') return d.datepicker['_' + a + 'Datepicker'].apply(d.datepicker,
[this[0]].concat(b)); return this.each(function() {
...
(function(b, d) {
  b.widget('ui.progressbar', {options:{value:0, max:100}, min:0, _create:function() {
    ...
    if (a === d) return this._value(); this._setOption('value', a); return this;
  }, _setOption:function(a, c) {
    if (a === 'value') {
      this.options.value = c; this._refreshValue(); this._value() === this.options.max && this._trigger('complete');
    }b.Widget.prototype._setOption.apply(this, arguments);
  }, _value:function() {
    var a = this.options.value; if (typeof a !== 'number')a = 0; return Math.min(this.options.max, Math.max(this.min, a));
...
this._value() / this.options.max;
    ...
    } this.valueDiv.toggleClass('ui-corner-right', a === this.options.max).width(c.toFixed(0) + '%'); this.element.attr('aria-valuenow', a);
...
16), parseInt(a[2], 16), parseInt(a[3], 16)]; if (a = /#([a-fA-F0-9])([a-fA-F0-9])([a-fA-F0-9])/.exec(c)) return [parseInt(a[1] + a[1], 16), parseInt(a[2] + a[2], 16), parseInt(a[3] + a[3], 16)]; if (/rgba\(0, 0, 0, 0\)/.exec(c)) return o.transparent; return o[f.trim(c).toLowerCase()];
    ...
    var b; do {
      b = f.curCSS(c, a); if (b != '' && b != 'transparent' || f.nodeName(c, 'body')) break; a = 'backgroundColor';
    } while (c = c.parentNode);return n(b);
  } function p() {
    var c = document.defaultView ? document.defaultView.getComputedStyle(this, null) : this.currentStyle,
      a = {}, b, d; if (c && c.length && c[0] && c[c[0]]) for (var e = c.length; e--;) {
    ...
    var a, b; for (a in c) {
      b = c[a]; if (b == null || f.isFunction(b) || a in t || /scrollbar/.test(a) || !/color/i.test(a) && isNaN(parseFloat(b))) delete c[a];
    } return c;
  } function u(c, a) {
    var b = {_:0}, d; for (d in a) if (c[d] != a[d])b[d] = a[d]; return b;
  } function k(c, a, b, d) {
    if (typeof c == 'object') {
      d =
a; b = null; a = c; c = a.effect;
    ...
    d = b; b = null;
  }a = a || {}; b = b || a.duration; b = f.fx.off ? 0 : typeof b == 'number' ? b : b in f.fx.speeds ? f.fx.speeds[b] : f.fx.speeds._default; d = d || a.complete; return [c, a, b, d];
  } function m(c) {
    if (!c || typeof c === 'number' || f.fx.speeds[c]) return true; if (typeof c === 'string' && !f.effects[c]) return true; return false;
  }f.effects = {}; f.each(['backgroundColor', 'borderBottomColor', 'borderLeftColor', 'borderRightColor',
    'borderTopColor', 'borderColor', 'color', 'outlineColor'], function(c, a) {
    f.fx.step[a] = function(b) {
    if (!b.colorInit) {
      b.start = s(b.elem, a); b.end = n(b.end); b.colorInit = true;
...
d) {
        if (f.isFunction(b)) {
          ...
          var e = f(this), g = e.attr('style') || ' ', h = q(p.call(this)), l, v = e.attr('className'); f.each(r, function(w, i) {
            c[i] && e[i + 'Class'](c[i]);
          }); l = q(p.call(this)); e.attr('className', v); e.animate(u(h, l), a, b, function() {
            f.each(r, function(w, i) {
              c[i] && e[i + 'Class'](c[i]);
            }); if (typeof e.attr('style') == 'object') {
              e.attr('style').cssText = ''; e.attr('style').cssText = g;
            } else e.attr('style', g); d && d.apply(this, arguments);
          }); h = f.queue(this); l = h.splice(h.length - 1, 1)[0];
...
a) {
  ...
  if (c.parent().is('.ui-effects-wrapper')) return c.parent(); var a = {width:c.outerWidth(true), height:c.outerHeight(true), 'float':c.css('float')}, b = f('<div></div>').addClass('ui-effects-wrapper').css({fontSize:'100%', background:'transparent',
      border:'none', margin:0, padding:0}); c.wrap(b); b = c.parent(); if (c.css('position') == 'static') {
        b.css({position:'relative'}); c.css({position:'relative'});
      } else {
        f.extend(a, {position:c.css('position'), zIndex:c.css('z-index')}); f.each(['top', 'left', 'bottom', 'right'], function(d, e) {
          a[e] = c.css(e); if (isNaN(parseInt(a[e], 10)))a[e] = 'auto';
  ...
  d = d || {}; f.each(a, function(e, g) {
    unit = c.cssUnit(g); if (unit[0] > 0)d[g] = unit[0] * b + unit[1];
  }); return d;
}}); f.fn.extend({effect:function(c) {
  var a = k.apply(this, arguments), b = {options:a[1], duration:a[2], callback:a[3]}; a = b.options.mode; var d = f.effects[c]; if (f.fx.off || !d) return a ? this[a](b.duration, b.callback) : this.each(function() {
    b.callback && b.callback.call(this);
  }); return d.call(this, b);
}, _show:f.fn.show, show:function(c) {
  if (m(c)) return this._show.apply(this, arguments);
  else {
    var a = k.apply(this, arguments); a[1].mode = 'show'; return this.effect.apply(this, a);
  }
}, _hide:f.fn.hide, hide:function(c) {
  if (m(c)) return this._hide.apply(this, arguments); else {
    var a = k.apply(this, arguments); a[1].mode = 'hide'; return this.effect.apply(this, a);
  }
  ...
  if (m(c) || typeof c === 'boolean' || f.isFunction(c)) return this.__toggle.apply(this, arguments); else {
    var a = k.apply(this, arguments); a[1].mode = 'toggle'; return this.effect.apply(this, a);
  }
}, cssUnit:function(c) {
  var a = this.css(c),
    b = []; f.each(['em', 'px', '%', 'pt'], function(d, e) {
      if (a.indexOf(e) > 0)b = [parseFloat(a), e];
    }); return b;
}}); f.easing.jswing = f.easing.swing; f.extend(f.easing, {def:'easeOutQuad', swing:function(c, a, b, d, e) {
  ...
  if ((a /= e / 2) < 1) return d / 2 * a * a + b; return -d / 2 * (--a * (a - 2) - 1) + b;
...
a, b, d, e) {
  ...
  if ((a /= e / 2) < 1) return d / 2 * a * a * a + b; return d / 2 * ((a -= 2) * a * a + 2) + b;
}, easeInQuart:function(c, a, b, d, e) {
  return d * (a /= e) * a * a * a + b;
}, easeOutQuart:function(c, a, b, d, e) {
  return -d * ((a = a / e - 1) * a * a * a - 1) + b;
}, easeInOutQuart:function(c, a, b, d, e) {
  if ((a /= e / 2) < 1) return d / 2 * a * a * a * a + b; return -d / 2 * ((a -= 2) * a * a * a - 2) + b;
...
a, b, d, e) {
  if ((a /= e / 2) < 1) return d / 2 * a * a * a * a * a + b; return d / 2 * ((a -= 2) * a * a * a * a + 2) + b;
...
e) return b + d; if ((a /= e / 2) < 1) return d / 2 * Math.pow(2, 10 * (a - 1)) + b; return d / 2 * (-Math.pow(2, -10 * --a) + 2) + b;
  ...
  if ((a /= e / 2) < 1) return -d / 2 * (Math.sqrt(1 - a * a) - 1) + b; return d / 2 * (Math.sqrt(1 - (a -= 2) * a) + 1) + b;
...
g / (2 * Math.PI) * Math.asin(d / h); return -(h * Math.pow(2, 10 * (a -= 1)) * Math.sin((a * e - c) * 2 * Math.PI / g)) + b;
  ...
  c = 1.70158; var g = 0, h = d; if (a == 0) return b; if ((a /= e) == 1) return b + d; g || (g = e * 0.3); if (h < Math.abs(d)) {
    h = d; c = g / 4;
  } else c = g / (2 * Math.PI) * Math.asin(d / h); return h * Math.pow(2, -10 * a) * Math.sin((a * e - c) * 2 * Math.PI / g) + d + b;
...
h); if (a < 1) return -0.5 * h * Math.pow(2, 10 * (a -= 1)) * Math.sin((a * e - c) * 2 * Math.PI / g) + b; return h * Math.pow(2, -10 * (a -= 1)) * Math.sin((a * e - c) * 2 * Math.PI / g) * 0.5 + d + b;
  ...
  if (g == j)g = 1.70158; return d * (a /= e) * a * ((g + 1) * a - g) + b;
}, easeOutBack:function(c, a, b, d, e, g) {
  if (g == j)g = 1.70158; return d * ((a = a / e - 1) * a * ((g + 1) * a + g) + 1) + b;
}, easeInOutBack:function(c, a, b, d, e, g) {
  if (g == j)g = 1.70158; if ((a /= e / 2) < 1) return d / 2 * a * a * (((g *= 1.525) + 1) * a - g) + b; return d / 2 * ((a -= 2) * a * (((g *= 1.525) + 1) * a + g) + 2) + b;
...
a, b, d, e) {
  ...
  if (a < e / 2) return f.easing.easeInBounce(c, a * 2, 0, d, e) * 0.5 + b; return f.easing.easeOutBounce(c, a * 2 - e, 0, d, e) * 0.5 + d * 0.5 + b;
}});
}(jQuery);
;/*
...
3); if (h == 'show')a.css('opacity', 0).css(f, d == 'pos' ? -c : c); if (h == 'hide')c /= m * 2; h != 'hide' && m--; if (h == 'show') {
  var g = {opacity:1}; g[f] = (d == 'pos' ? '+=' : '-=') + c; a.animate(g, i / 2, b.options.easing); c /= 2; m--;
} for (g = 0; g < m; g++) {
  var j = {}, k = {}; j[f] = (d == 'pos' ? '-=' : '+=') + c; k[f] = (d == 'pos' ? '+=' : '-=') + c; a.animate(j, i / 2, b.options.easing).animate(k, i / 2, b.options.easing); c = h == 'hide' ? c * 2 : c / 2;
} if (h == 'hide') {
  ...
  j = {}; k = {}; j[f] = (d == 'pos' ? '-=' : '+=') + c; k[f] = (d == 'pos' ? '+=' : '-=') + c; a.animate(j, i / 2, b.options.easing).animate(k, i / 2, b.options.easing, function() {
    e.effects.restore(a, l); e.effects.removeWrapper(a); b.callback && b.callback.apply(this, arguments);
  });
}a.queue('fx', function() {
  a.dequeue();
}); a.dequeue();
...
c / 2);
      } var h = {}; h[g.size] = f == 'show' ? c : 0; h[g.position] = f == 'show' ? 0 : c / 2; d.animate(h, {queue:false, duration:e.duration, easing:e.options.easing, complete:function() {
        f == 'hide' && a.hide(); b.effects.restore(a, i); b.effects.removeWrapper(a); e.callback && e.callback.apply(a[0], arguments); a.dequeue();
      }});
    });
  };
})(jQuery);
...
'show' ? 1 : 0}; i[f] = (e == 'show' ? b == 'pos' ? '+=' : '-=' : b == 'pos' ? '-=' : '+=') + g; a.animate(i, {queue:false, duration:d.duration, easing:d.options.easing, complete:function() {
  e == 'hide' && a.hide(); c.effects.restore(a, h); c.effects.removeWrapper(a); d.callback && d.callback.apply(this, arguments); a.dequeue();
}});
    });
  };
})(jQuery);
...
e * (i / c) + (a.options.mode == 'show' ? 0 : (e - Math.floor(c / 2)) * (i / c)), opacity:a.options.mode == 'show' ? 1 : 0}, a.duration || 500); setTimeout(function() {
  a.options.mode == 'show' ? b.css({visibility:'visible'}) : b.css({visibility:'visible'}).hide(); a.callback && a.callback.apply(b[0]); b.dequeue(); j('div.ui-effects-explode').remove();
}, a.duration || 500);
    });
  };
})(jQuery);
...
(function(b) {
  b.effects.fade = function(a) {
    return this.queue(function() {
      var c = b(this), d = b.effects.setMode(c, a.options.mode || 'hide'); c.animate({opacity:d}, {queue:false, duration:a.duration, easing:a.options.easing, complete:function() {
        a.callback && a.callback.apply(this, arguments); c.dequeue();
      }});
    });
  };
})(jQuery);
...
10) / 100 * f[d == 'hide' ? 0 : 1]; if (d == 'show')e.css(h ? {height:0, width:g} : {height:g, width:0}); h = {}; i = {}; h[l[0]] = d == 'show' ? f[0] : g; i[l[1]] = d == 'show' ? f[1] : 0; e.animate(h, k, a.options.easing).animate(i, k, a.options.easing, function() {
  d == 'hide' && b.hide(); c.effects.restore(b, j); c.effects.removeWrapper(b); a.callback && a.callback.apply(b[0], arguments); b.dequeue();
});
    });
  };
})(jQuery);
  ...
  d.effects.pulsate = function(a) {
    return this.queue(function() {
      var b = d(this), c = d.effects.setMode(b, a.options.mode || 'show'); times = (a.options.times || 5) * 2 - 1; duration = a.duration ? a.duration / 2 : d.fx.speeds._default / 2; isVisible = b.is(':visible'); animateTo = 0; if (!isVisible) {
        b.css('opacity', 0).show(); animateTo = 1;
      } if (c == 'hide' && isVisible || c == 'show' && !isVisible)times--; for (c = 0; c < times; c++) {
        b.animate({opacity:animateTo}, duration, a.options.easing); animateTo = (animateTo + 1) % 2;
      }b.animate({opacity:animateTo}, duration,
a.options.easing, function() {
  animateTo == 0 && b.hide(); a.callback && a.callback.apply(this, arguments);
}); b.queue('fx', function() {
  b.dequeue();
}).dequeue();
    });
...
(function(c) {
  c.effects.puff = function(b) {
    return this.queue(function() {
      var a = c(this), e = c.effects.setMode(a, b.options.mode || 'hide'), g = parseInt(b.options.percent, 10) || 150, h = g / 100, i = {height:a.height(), width:a.width()}; c.extend(b.options, {fade:true, mode:e, percent:e == 'hide' ? g : 100, from:e == 'hide' ? i : {height:i.height * h, width:i.width * h}}); a.effect('scale', b.options, b.duration, b.callback); a.dequeue();
    });
...
}f = {height:a.height(), width:a.width()}; a.from = b.options.from || (g == 'show' ? {height:0, width:0} : f); h = {y:i != 'horizontal' ? h / 100 : 1, x:i != 'vertical' ? h / 100 : 1}; a.to = {height:f.height * h.y, width:f.width * h.x}; if (b.options.fade) {
  if (g == 'show') {
...
1; a.to.opacity = 0;
    ...
    return this.queue(function() {
      var a = c(this), e = ['position', 'top', 'bottom', 'left', 'right', 'width', 'height', 'overflow', 'opacity'], g = ['position', 'top', 'bottom', 'left', 'right', 'overflow', 'opacity'], h = ['width', 'height', 'overflow'], i = ['fontSize'], f = ['borderTopWidth', 'borderBottomWidth', 'paddingTop', 'paddingBottom'], k = ['borderLeftWidth', 'borderRightWidth', 'paddingLeft', 'paddingRight'],
        p = c.effects.setMode(a, b.options.mode || 'effect'), n = b.options.restore || false, m = b.options.scale || 'both', l = b.options.origin, j = {height:a.height(), width:a.width()}; a.from = b.options.from || j; a.to = b.options.to || j; if (l) {
          l = c.effects.getBaseline(l, j); a.from.top = (j.height - a.from.height) * l.y; a.from.left = (j.width - a.from.width) * l.x; a.to.top = (j.height - a.to.height) * l.y; a.to.left = (j.width - a.to.width) * l.x;
        } var d = {from:{y:a.from.height / j.height, x:a.from.width / j.width}, to:{y:a.to.height / j.height, x:a.to.width / j.width}};
      if (m == 'box' || m == 'both') {
        if (d.from.y != d.to.y) {
          e = e.concat(f); a.from = c.effects.setTransition(a, f, d.from.y, a.from); a.to = c.effects.setTransition(a, f, d.to.y, a.to);
        } if (d.from.x != d.to.x) {
          e = e.concat(k); a.from = c.effects.setTransition(a, k, d.from.x, a.from); a.to = c.effects.setTransition(a, k, d.to.x, a.to);
        }
      } if (m == 'content' || m == 'both') if (d.from.y != d.to.y) {
        e = e.concat(i); a.from = c.effects.setTransition(a, i, d.from.y, a.from); a.to = c.effects.setTransition(a, i, d.to.y, a.to);
      }c.effects.save(a, n ? e : g); a.show(); c.effects.createWrapper(a);
      a.css('overflow', 'hidden').css(a.from); if (m == 'content' || m == 'both') {
        f = f.concat(['marginTop', 'marginBottom']).concat(i); k = k.concat(['marginLeft', 'marginRight']); h = e.concat(f).concat(k); a.find('*[width]').each(function() {
          child = c(this); n && c.effects.save(child, h); var o = {height:child.height(), width:child.width()}; child.from = {height:o.height * d.from.y, width:o.width * d.from.x}; child.to = {height:o.height * d.to.y, width:o.width * d.to.x}; if (d.from.y != d.to.y) {
            ...
            child.from = c.effects.setTransition(child, k, d.from.x, child.from); child.to = c.effects.setTransition(child, k, d.to.x, child.to);
...
(h == 'pos' ? '-=' : '+=') + e * 2; b.animate(c, f, a.options.easing); for (e = 1; e < l; e++)b.animate(i, f, a.options.easing).animate(k, f, a.options.easing); b.animate(i, f, a.options.easing).animate(c, f / 2, a.options.easing, function() {
  d.effects.restore(b, j); d.effects.removeWrapper(b); a.callback && a.callback.apply(this, arguments);
}); b.queue('fx', function() {
  b.dequeue();
}); b.dequeue();
    });
...
(function(c) {
  c.effects.slide = function(d) {
    return this.queue(function() {
      ...
      var i = {}; i[g] = (f == 'show' ? b == 'pos' ? '+=' : '-=' : b == 'pos' ? '-=' : '+=') + e; a.animate(i, {queue:false, duration:d.duration, easing:d.options.easing, complete:function() {
        f == 'hide' && a.hide(); c.effects.restore(a, h); c.effects.removeWrapper(a); d.callback && d.callback.apply(this, arguments); a.dequeue();
      }});
    });
  };
})(jQuery);
